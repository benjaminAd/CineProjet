...
<div class="main">
    ...
    <form method="POST" action="${pageContext.request.contextPath}/AddSalle?formid=${salle.getId()}&Film=${salle.getFilm().getNomFilm()}">
        ...
        <div class="form-group">
			...
			<input type="number" class="form-control" name="places" required="required" id="places"/>
        </div>
		<div class="form-group">
			<select class="custom-select custom-select-lg mb-3" name="film" size="1" >
				<option value="" disabled selected>Choisir un film</option>
				 <c:forEach items="${films}" var="film">
				  	<c:if test="${film.isActif() == 0 }"><option value="${film.getId()}">${film.getNomFilm()}</option></c:if>
				  </c:forEach>
  			</select>
        </div>
		<div class="form-group">
		...
		<p><c:if test="${param.salle == 'True'}">Cette Salle est déjà utilisée!</c:if></p>
    </form>
</div>
					</div>
